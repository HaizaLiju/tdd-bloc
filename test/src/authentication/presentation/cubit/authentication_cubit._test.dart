...
void main() {
  ...
  const tAPITFailure = ApiFailure(message: 'message', statusCode: 400);
  ...
  group('createUser', () {
    ...
    blocTest<AuthenticationCubit, AuthenticationState>(
      ...
      build: () {
        when(() => createUser(any())).thenAnswer(
          (_) async => const Left(tAPITFailure),
        );
        return cubit;
      },
      act: (cubit) => cubit.createUser(
      ...
      expect: () => [
        const CreatingUser(),
        AuthenticationError(tAPITFailure.errorMessage),
      ],
      verify: (_) {
        verify(() => createUser(tCreateUserParams)).called(1);
        verifyNoMoreInteractions(createUser);
  ...
  group('getUsers', () {
    ...
    blocTest<AuthenticationCubit, AuthenticationState>(
        'should emit [GettingUsers, AuthenticationError] when successful',
        build: () {
          when(() => getUsers()).thenAnswer((_) async => Left(tAPITFailure));
          return cubit;
        },
        act: (cubit) => cubit.getUsers(),
        expect: () => [
